...
create table profiles (
  ...
  avatar_url text,
  elo integer default 1000 check (elo >= 0 and elo <= 9999),
...
-- TRIVIA QUESTIONS TABLE (without foreign key to answers - will add after answers table)
...
-- Add correct_answer_id to trivia_questions (after answers table exists)
...
create table leaderboard_snapshots (
  ...
  elo integer not null check (elo >= 0 and elo <= 9999),
  created_at timestamptz default now(),
...
create table notifications (
  id uuid primary key default uuid_generate_v4(),
  user_id uuid not null references profiles(id) on delete cascade,
  type text not null check (type in ('match_invite', 'match_result', 'friend_request', 'system')),
...
returns trigger as $$
begin
  insert into public.profiles (user_id, username, elo)
  values (
    new.id,
